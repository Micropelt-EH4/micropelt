    ...
    match &s.fields {
        syn::Fields::Named(named) => {
            for (i, field) in named.named.iter().enumerate() {
                ...
                if let Some(resolution) = attributes_get_resoultion(&field.attrs) {
                    inner.extend(quote!((self.#name - other.#name).abs() < #resolution))
                } else {
                    inner.extend(quote!(self.#name == other.#name))
                }
            }
        }
        _ => unimplemented!(
